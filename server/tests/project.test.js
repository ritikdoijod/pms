...
describe("POST /workspaces", () => {
  ...
  it(
    "should not create a workspace and return a 400 status if the request body is empty",
    async () => {
      const res = await request(app)
        .post("/workspaces/")
        .set("Authorization", `Bearer ${user1Token}`)
...
describe("PATCH /workspaces", () => {
  it("should return a updated workspace with 200 status code for a authorized user", async () => {
    const res = await request(app).patch(`/workspaces/${workspaces[0]._id}`)
      .set("Authorization", `Bearer ${user1Token}`)
      .set("Accept", "application/json")
      .send({
        name: "Workspace5"
  ...
  it("should return a 404 status if workspace not found", async () => {
    const res = await request(app).patch(`/workspaces/${new mongoose.Types.ObjectId().toString()}`)
      .set("Authorization", `Bearer ${user1Token}`)
      .set("Accept", "application/json")
      .send({
        name: "Workspace7"
...
describe("DELETE /workspaces", () => {
  it("should delete a workspace and return 200 status code for a authorized user", async () => {
    const res = await request(app).delete(`/workspaces/${workspaces[0]._id}`)
      .set("Authorization", `Bearer ${user1Token}`)
      .set("Accept", "application/json");
  ...
  it("should return a 403 status for unauthorized user", async () => {
    const res = await request(app).delete(`/workspaces/${workspaces[1]._id}`)
      .set("Authorization", `Bearer ${user2Token}`)
      .set("Accept", "application/json");
